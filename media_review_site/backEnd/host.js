...
app.get('/api/getMusicReviews', async (req, res) => {
    try {
        const musicReviews = await MusicReviewModel.find();
...
app.post('/api/addMovieReview', async (req, res) => {
    try {
        const newMovieReview = new MovieReviewModel(req.body);
...
app.post('/api/addMusicReview', async (req, res) => {
    try {
        const newMusicReview = new MusicReviewModel(req.body);
        const savedReview = await newMusicReview.save();
        res.status(201).send(savedReview);
    } catch (err) {
